...
namespace Assets.Scripts.Game.Managers
{
    public class GameManager : NetworkBehaviour
    {
        ...
        private void StartCountdown(float seconds)
        {
            ...
            isCountingDown = true;
            Debug.Log($"Started countdown: {seconds} seconds");
        }

        private void AdvanceGameState()
        {
            // Only server controls game state
            if (!IsServer) return;
        ...
        private void EndRound(int winningTeam)
        {
            ...
            // Start end round countdown
            StartCountdown(roundEndCountdown);

            Debug.Log($"Round ended - Team {winningTeam} wins");
        }
        ...
        private void EndGame()
        {
            ...
            // Determine winner
            int finalWinner = team1Score.Value > team2Score.Value ? 1 : 2;
            Debug.Log($"Game ended - Team {finalWinner} wins ({team1Score.Value}-{team2Score.Value})");
        }

        private bool ShouldEndGame()
        {
            // Get max rounds from lobby manager
            int maxRounds = GameLobbyManager.Instance != null ?
                GameLobbyManager.Instance.RoundCount : 3;  // Default to 3 rounds

            // Calculate required wins
            int requiredWins = (maxRounds / 2) + 1;

            // Check game end conditions
            bool maxRoundsReached = currentRound.Value >= maxRounds;
            bool team1Victory = team1Score.Value >= requiredWins;
            bool team2Victory = team2Score.Value >= requiredWins;

            Debug.Log($"Game end check - Current round: {currentRound.Value}/{maxRounds}, " +
                      $"Team 1: {team1Score.Value}/{requiredWins}, Team 2: {team2Score.Value}/{requiredWins}");

            return maxRoundsReached || team1Victory || team2Victory;
        }
        #endregion
        ...
        {
            // Route to appropriate method based on network role
            if (IsServer)
        ...
        {
            // Only process if round in progress
            if (gameState.Value != GameState.RoundInProgress) return;
